...
public class FindStudent extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView=findViewById(R.id.findtext);
        cview=findViewById(R.id.findcard);
        studentDatabaseHelper=new StudentDatabaseHelper(this);
        database=studentDatabaseHelper.getReadableDatabase();
        findViewById(R.id.findbtn).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Cursor c=database.rawQuery("SELECT * FROM student_table WHERE REG='"+editText.getText()+"'",null);
                if(c.moveToFirst()){
                    cview.setVisibility(View.VISIBLE);
                    textView.setText("Reg: "+c.getString(1)+"\n"+"Name: "+c.getString(2)+"\n"+
                            "Email: "+c.getString(3)+"\n"+"Phone: "+c.getString(4)+"\n"+"Stream: "+c.getString(5)+"\n");
                }
                else{
